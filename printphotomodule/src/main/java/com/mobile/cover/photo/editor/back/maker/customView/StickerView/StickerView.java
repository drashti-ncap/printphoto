...
//                Log.e("TAG","DRAG Stickers");
    ...
    private void transformSticker(Sticker sticker) {
        ...
        mSizeMatrix.postScale(scaleFactor, scaleFactor,
    ...
    public void updateSticker(Sticker sticker) {
        ...
        Log.e("sticker123123", "scaleFactor==>" + scaleFactor);
        Log.e("sticke123123r", "getWidth()==>" + getWidth());
        Log.e("s123123ticker", "getHeight()()==>" + getHeight());
        Log.e("sti123cker", " sticker.getDrawable().getIntrinsicWidth()==>" + sticker.getDrawable().getIntrinsicWidth());
        Log.e("123", " sticker.getDrawable().getIntrinsicHeight()==>" + sticker.getDrawable().getIntrinsicHeight());
        Log.e("st123123icker", " sticker.getMatrix()==>" + sticker.getMatrix());
    ...
    public void addfancyStickerMain(Sticker sticker) {
        if (sticker == null) {
            Log.e(TAG, "Sticker to be added is null!");
            return;
        }
...
//
//        scaleFactor = (float) (scaleFactor * 1.3);
//        sticker.getMatrix().postScale(Share.maskwidth * 100, Share.maskheight * 100, getWidth() / 2, getHeight() / 2);
    ...
    public void addStickerMain_bg(Sticker sticker, int pos) {
        ...
        mHandlingSticker = sticker;
        if (mStickers.size() == 0) {
            Log.e(TAG, "addStickerMain_bg: " + mStickers.size());
            mStickers.add(sticker);
        } else {
            Log.e(TAG, "addStickerMain_bg:=====>else " + mStickers.size());
...
//        mMidPoint = calculateMidPoint();
//        mHandlingSticker = sticker;
//        //  mHandlingSticker.getMatrix().postScale(1.5f, 1.5f, getWidth() / 2, getHeight() / 2);
//        mStickers.add(pos, sticker);
//        if (mOnStickerOperationListener != null) {
//            mOnStickerOperationListener.onStickerAdd(mHandlingSticker);
//        }
...
//    }
    ...
    public void edited_addStickerMain(Sticker sticker) {
        if (sticker == null) {
            Log.e(TAG, "Sticker to be added is null!");
            return;
        }
        float offsetX = Math.abs((getWidth() - sticker.getWidth()) / 2);
        float offsetY = Math.abs((getHeight() - sticker.getHeight()) / 2);
        Log.e(TAG, "edited_addStickerMain: " + offsetX + "======>" + offsetY);
        sticker.getMatrix().postTranslate(offsetX, offsetY);
...
//        }
    ...
    public void dynamic_edited_addStickerMain(Sticker sticker) {
        if (sticker == null) {
            Log.e(TAG, "Sticker to be added is null!");
            return;
        }
        float offsetX = (getWidth()) / 2;
        float offsetY = (getHeight()) / 2;
        Log.e(TAG, "edited_addStickerMain: ===============>dynamic" + offsetX + "======>" + offsetY);
        sticker.getMatrix().postTranslate(offsetX, offsetY);
...
//        sticker.getMatrix().postScale(Share.maskheight*3/2, Share.maskheight*3/2, getWidth()/2 , getHeight() /2);
    ...
    public Bitmap createBitmap1(float width, float height) {
        mHandlingSticker = null;
        Bitmap bitmap = Bitmap.createBitmap((int) width * 200, (int) height * 200, Bitmap.Config.ARGB_8888);
