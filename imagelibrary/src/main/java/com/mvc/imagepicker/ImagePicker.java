...
//            Uri uri=FileProvider.getUriForFile(context, BuildConfig.APPLICATION_ID,getTemporalFile(context));
//            takePhotoIntent.putExtra(MediaStore.EXTRA_OUTPUT,uri);
    ...
    public static Uri getImageFromResult(Context context, int requestCode, int resultCode,
        ...
        Log.e("CHECKDIALOG", "getImageFromResult() called with: " + "resultCode = [" + resultCode + "] requestCode = [" + requestCode + "]");
        Bitmap bm = null;
        Uri selectedImage = null;
        if (resultCode == Activity.RESULT_OK && requestCode == PICK_IMAGE_REQUEST_CODE) {
            Log.e("CHECKDIALOG", "getImageFromResult() called with: data-->" + imageReturnedIntent);
            File imageFile = getTemporalFile(context);

            boolean isCamera = (imageReturnedIntent == null
                    || imageReturnedIntent.getData() == null
                    || imageReturnedIntent.getData().toString().contains(imageFile.toString()));
            Log.e("CHECKDIALOG", "getImageFromResult: isCamera-->" + isCamera);
            if (isCamera) {     /** CAMERA **/
                selectedImage = Uri.fromFile(imageFile);
//                selectedImage = FileProvider.getUriForFile(context, BuildConfig.APPLICATION_ID, imageFile);
            } else {            /** ALBUM **/
                selectedImage = imageReturnedIntent.getData();
            }
            Log.e("CHECKDIALOG", "selectedImage: " + selectedImage);
    ...
    private static File getTemporalFile(Context context) {
        return new File(context.getExternalCacheDir().getAbsolutePath()/*, TEMP_IMAGE_NAME*/);
    }
